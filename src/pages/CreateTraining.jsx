...
const CreateTraining = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('✅ Training created:', newTraining);
      
      if (newTraining && newTraining.id) {
        console.log('🔄 Navigating to training:', newTraining.id);
        navigate(`/trainings/${newTraining.id}`);
      } else {
        console.error('❌ No ID returned from createTraining');
        navigate('/');
      }
    } catch (error) {
      console.error('❌ Error creating training:', error);
      alert('Fehler beim Erstellen des Trainings: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
