...
const { FiArrowLeft, FiPlus, FiEdit, FiTrash2, FiCalendar, FiTarget, FiFileText, FiCopy } = FiIcons;

const TopicDetail = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { getTopic, getLearningUnitsByTopic, deleteLearningUnit, createLearningUnit, EDITORIAL_STATES } = useLearningUnits();
  ...
  const handleDuplicate = (unit) => {
    const duplicatedUnit = {
      ...unit,
      id: undefined, // Will be generated by createLearningUnit
      title: `${unit.title} (Kopie)`,
      createdAt: undefined, // Will be set by createLearningUnit
      updatedAt: undefined, // Will be set by createLearningUnit
      editorialState: EDITORIAL_STATES.PLANNING, // Reset to planning state
      // Keep all other data including textSnippets, images, etc.
      textSnippets: unit.textSnippets ? unit.textSnippets.map(snippet => ({
        ...snippet,
        id: Date.now() + Math.random(), // Generate new IDs for snippets
        createdAt: new Date().toISOString(),
        rating: { up: 0, down: 0, userVotes: {} }, // Reset ratings
        comments: [], // Clear comments
        approved: false // Reset approval
      })) : [],
      // Clear video and file references as they can't be easily duplicated
      video: null,
      powerPointFile: null,
      // Keep images but note they reference the same URLs
      images: unit.images || [],
      // Clear comments
      comments: []
    };

    const newUnit = createLearningUnit(duplicatedUnit);
    alert(`Lerneinheit "${unit.title}" wurde erfolgreich dupliziert!`);
              ...
              <div className="flex justify-between items-center">
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    className="inline-flex items-center px-3 py-2 bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 rounded-lg hover:bg-green-200 dark:hover:bg-green-800 transition-colors text-sm"
                    title="Lerneinheit duplizieren"
                  >
